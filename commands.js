...
const Commands = {
    nextevent: (content) => {
      // returns a promise
      return Meetup.getGroupEvents(config.urlname)
        .then(function(events) {
          // get first returned event
          let event = events[0];
          // TODO: check if this is corrent for empty events
          if (!event) return 'No upcoming events. Check back later.';


          // Finds if there is a number specified
          let number = content.match(/\d+/);
          // If theres no number assume 1
          number === null ? number = 1 : number = number[0];
          // Create intro string
          let str = `**Next up coming event${number > 1 ? 's**:\n' : '**: '}`
          let eventStr = '';
